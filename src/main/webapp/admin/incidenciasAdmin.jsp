...
<head>
    ...
    <!-- Estilos propios -->
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/admin/incidenciasAdmin.css">
</head>
<body>
   ...
   <div class="panel-container container-fluid">
    ...
    <!-- Barra de filtros -->
    <div class="mb-3">
       <a href="${pageContext.request.contextPath}/admin/incidencia?filtro=pendientes"
          class="btn btn-filtro${filtroActual == 'pendientes' ? ' btn-activo' : ''}">
           Pendientes
       </a>
       <a href="${pageContext.request.contextPath}/admin/incidencia?filtro=en_proceso"
          class="btn btn-filtro${filtroActual == 'en_proceso' ? ' btn-activo' : ''}">
           En Proceso
       </a>
       <a href="${pageContext.request.contextPath}/admin/incidencia?filtro=completadas"
          class="btn btn-filtro${filtroActual == 'completadas' ? ' btn-activo' : ''}">
           Completadas
       </a>
       <a href="${pageContext.request.contextPath}/admin/incidencia?filtro=todas"
          class="btn btn-filtro${filtroActual == 'todas' ? ' btn-activo' : ''}">
           Todas
       </a>
       <a href="${pageContext.request.contextPath}/admin/incidencia?filtro=mias"
          class="btn btn-filtro${filtroActual == 'mias' ? ' btn-activo' : ''}">
           Mías
       </a>
    </div>
           ...
           <tbody>
               <c:forEach var="incidencia" items="${incidencias}">
                   <tr>
                       ...
                       <td class="td-descripcion" title="${incidencia.descripcion}"><div class="descripcion-scroll">${incidencia.descripcion}</div></td>
                       <td><fmt:formatDate value="${incidencia.fechaCreacionDate}" pattern="dd-MM-yyyy HH:mm"/></td>
                       <td>
                           <span class="badge badge-estado ${incidencia.estado}">${incidencia.estado}</span>
                       </td>
                       <td>
                           <button class="btn btn-accion-azul" data-bs-toggle="modal" data-bs-target="#modalEstadoIncidencia${incidencia.id}">
       ...
       <div class="modal-dialog">
         <div class="modal-content">
           <form method="post" action="${pageContext.request.contextPath}/admin/incidencia">
             ...
             <div class="modal-footer">
               <button type="button" class="btn btn-accion-gris" data-bs-dismiss="modal">Cancelar</button>
               <button type="submit" class="btn btn-accion-azul">Crear Incidencia</button>
             </div>
           </form>
         </div>
       </div>
   ...
   <div class="modal fade" id="modalNuevaIncidencia" tabindex="-1" aria-labelledby="modalNuevaIncidenciaLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <form method="post" action="${pageContext.request.contextPath}/operario/incidencia">
            <input type="hidden" name="accion" value="crear" />
            <div class="modal-header">
              <h5 class="modal-title" id="modalNuevaIncidenciaLabel">Nueva Incidencia</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
