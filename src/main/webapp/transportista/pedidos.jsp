...
<head>
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/transportista/pedidoTransportista.css">
</head>
<body>
                    ...
                    <div class="collapse navbar-collapse justify-content-center" id="navbarTransportista">
                        <ul class="navbar-nav flex-row w-100 justify-content-center align-items-center">
                            <li class="nav-item d-none d-md-block d-lg-none mx-2">
                                <a class="nav-link text-center" href="${pageContext.request.contextPath}/transportista/pedidos">
    ...
    <div class="panel-container container-fluid">
      ...
      <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <!-- Botón simulado para nuevo pedido, oculto por ahora -->
        <button class="btn btn-nueva d-none"><i class="fas fa-plus"></i> Nuevo Pedido</button>
      </div>
        <c:choose>
                    ...
                    <tbody>
                        <c:forEach var="pedido" items="${pedidosProcesados}">
                            <tr>
                                ...
                                <td>
                                    <form action="${pageContext.request.contextPath}/transportista/pedidos" method="post">
                                        <input type="hidden" name="accion" value="asignarTransportista">
        ...
        <h2>Mis pedidos en envío</h2>
        <c:choose>
            <c:when test="${empty pedidosEnviando}">
                No tienes pedidos en envío actualmente.
            </c:when>
            <c:otherwise>
                <table class="table table-striped">
                    <thead>
                    ...
                    <tbody>
                        <c:forEach var="pedido" items="${pedidosEnviando}">
                            <tr>
                                <td>${pedido.id}</td>
                                <td>${pedido.nombreProducto}</td>
                                <td>${pedido.productoId}</td>
                                ...
                                <td>
                                    <form action="${pageContext.request.contextPath}/transportista/pedidos" method="post">
                                        <input type="hidden" name="accion" value="marcarEntregado">
                                        <input type="hidden" name="pedidoId" value="${pedido.id}">
                                        <input type="hidden" name="usuarioId" value="${sessionScope.usuario.id}">
                                        <button type="submit" class="btn btn-accion-azul btn-sm">Marcar como entregado</button>
                                    </form>
                                </td>
                            </tr>
                        </c:forEach>
        ...
        <table class="table table-striped">
            ...
            <tbody>
                <c:forEach var="pedido" items="${pedidosEntregados}">
                    <tr>
                        <td>${pedido.id}</td>
                        <td>${pedido.nombreProducto}</td>
                        <td>${pedido.productoId}</td>
