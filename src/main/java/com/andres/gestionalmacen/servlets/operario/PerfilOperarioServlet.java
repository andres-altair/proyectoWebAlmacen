...
/**
 * Servlet que gestiona el perfil del operario.
 * 
 * @author andres
 */
@WebServlet("/operario/perfil")
@MultipartConfig // para soporte de subida de foto
public class PerfilOperarioServlet extends HttpServlet {
    private UsuarioServicio usuarioServicio = new UsuarioServicio();
    /**
     * Maneja las peticiones GET para mostrar el perfil del operario autenticado.
     * 
    ...
    protected void doGet(HttpServletRequest peticion, HttpServletResponse respuesta) throws ServletException, IOException {
        ...
        // Solo operarios (rolId=3)
        if (usuario.getRolId() != 3) {
            respuesta.sendRedirect(peticion.getContextPath() + "/acceso");
            return;
        }
        GestorRegistros.info(usuario.getId(), "Acceso al perfil de operario.");
        try {
            peticion.setAttribute("usuario", usuario);
            peticion.getRequestDispatcher("/operario/perfilOperario.jsp").forward(peticion, respuesta);
        } catch (Exception e) {
            GestorRegistros.error(usuario.getId(), "Error al cargar el perfil de operario: " + e.getMessage());
            peticion.setAttribute("error", "No se pudo cargar el perfil: " + e.getMessage());
            peticion.getRequestDispatcher("/operario/perfilOperario.jsp").forward(peticion, respuesta);
        }
    }
    /**
     * Maneja las peticiones POST para actualizar el perfil del operario.
     * 
     * @param peticion objeto que contiene la petición HTTP
     * @param respuesta objeto que contiene la respuesta HTTP
     * @throws ServletException si ocurre un error en el servlet
        ...
        try {
            ...
            if (parteFoto != null && parteFoto.getSize() > 0) {
                ...
                try {
                    ...
                    respuesta.sendRedirect(peticion.getContextPath() + "/operario/perfilOperario.jsp");
                    return;
                }
            }
            ...
            GestorRegistros.info(usuarioId, "Perfil de operario actualizado correctamente.");
            peticion.setAttribute("usuario", usuario);
            peticion.setAttribute("mensaje", "Perfil actualizado correctamente");
            peticion.getRequestDispatcher("/operario/perfilOperario.jsp").forward(peticion, respuesta);
        } catch (Exception e) {
            GestorRegistros.error(usuarioId, "Error al actualizar el perfil de operario: " + e.getMessage());
